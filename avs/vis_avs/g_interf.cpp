...
int win32_dlgproc_interferences(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM) {
    ...
    switch (uMsg) {
        case WM_INITDIALOG:
            ...
            SendDlgItemMessage(
                hwndDlg, IDC_NPOINTS, TBM_SETRANGE, TRUE, MAKELONG(0, 8));
            SendDlgItemMessage(
                hwndDlg, IDC_NPOINTS, TBM_SETPOS, TRUE, g_ConfigThis->nPoints);
            SendDlgItemMessage(hwndDlg, IDC_ALPHA, TBM_SETTICFREQ, 16, 0);
            SendDlgItemMessage(
                hwndDlg, IDC_ALPHA, TBM_SETRANGE, TRUE, MAKELONG(1, 255));
            SendDlgItemMessage(
                hwndDlg, IDC_ALPHA, TBM_SETPOS, TRUE, g_ConfigThis->alpha);
            SendDlgItemMessage(hwndDlg, IDC_DISTANCE, TBM_SETTICFREQ, 8, 0);
            SendDlgItemMessage(
                hwndDlg, IDC_DISTANCE, TBM_SETRANGE, TRUE, MAKELONG(1, 64));
            SendDlgItemMessage(
                hwndDlg, IDC_DISTANCE, TBM_SETPOS, TRUE, g_ConfigThis->distance);
            SendDlgItemMessage(hwndDlg, IDC_ROTATE, TBM_SETTICFREQ, 2, 0);
            SendDlgItemMessage(
                hwndDlg, IDC_ROTATE, TBM_SETRANGE, TRUE, MAKELONG(0, 64));
            SendDlgItemMessage(hwndDlg,
                               IDC_ROTATE,
                               TBM_SETPOS,
                               TRUE,
                               g_ConfigThis->rotationinc * -1 + 32);
            SendDlgItemMessage(hwndDlg, IDC_ALPHA2, TBM_SETTICFREQ, 16, 0);
            SendDlgItemMessage(
                hwndDlg, IDC_ALPHA2, TBM_SETRANGE, TRUE, MAKELONG(1, 255));
            SendDlgItemMessage(
                hwndDlg, IDC_ALPHA2, TBM_SETPOS, TRUE, g_ConfigThis->alpha2);
            SendDlgItemMessage(hwndDlg, IDC_DISTANCE2, TBM_SETTICFREQ, 8, 0);
            SendDlgItemMessage(
                hwndDlg, IDC_DISTANCE2, TBM_SETRANGE, TRUE, MAKELONG(1, 64));
            SendDlgItemMessage(
                hwndDlg, IDC_DISTANCE2, TBM_SETPOS, TRUE, g_ConfigThis->distance2);
            SendDlgItemMessage(hwndDlg, IDC_ROTATE2, TBM_SETTICFREQ, 2, 0);
            SendDlgItemMessage(
                hwndDlg, IDC_ROTATE2, TBM_SETRANGE, TRUE, MAKELONG(0, 64));
            SendDlgItemMessage(hwndDlg,
                               IDC_ROTATE2,
                               TBM_SETPOS,
                               TRUE,
                               g_ConfigThis->rotationinc2 * -1 + 32);
            SendDlgItemMessage(hwndDlg, IDC_INITROT, TBM_SETTICFREQ, 16, 0);
            SendDlgItemMessage(
                hwndDlg, IDC_INITROT, TBM_SETRANGE, TRUE, MAKELONG(0, 255));
            SendDlgItemMessage(
                hwndDlg, IDC_INITROT, TBM_SETPOS, TRUE, 255 - g_ConfigThis->rotation);
            SendDlgItemMessage(hwndDlg, IDC_SPEED, TBM_SETTICFREQ, 1000, 0);
            SendDlgItemMessage(
                hwndDlg, IDC_SPEED, TBM_SETRANGE, TRUE, MAKELONG(1, 128));
            SendDlgItemMessage(
                hwndDlg, IDC_SPEED, TBM_SETPOS, TRUE, (int)(g_ConfigThis->speed * 100));
            ...
            EnableWindow(GetDlgItem(hwndDlg, IDC_RGB),
                         (g_ConfigThis->nPoints == 3 || g_ConfigThis->nPoints == 6));
            CheckDlgButton(
                hwndDlg, IDC_RGB, g_ConfigThis->rgb ? BST_CHECKED : BST_UNCHECKED);
            CheckDlgButton(hwndDlg,
                           IDC_ONBEAT,
                           g_ConfigThis->onbeat ? BST_CHECKED : BST_UNCHECKED);
            return 1;
        case WM_COMMAND:
            if ((LOWORD(wParam) == IDC_CHECK1) || (LOWORD(wParam) == IDC_RGB)
                || (LOWORD(wParam) == IDC_ONBEAT) || (LOWORD(wParam) == IDC_ADDITIVE)
