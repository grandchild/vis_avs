...
int win32_dlgproc_bump(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    const Parameter* p_on_beat = &g_this->info.parameters[4];
    const Parameter* p_on_beat_duration = &g_this->info.parameters[5];
    const Parameter* p_on_beat_depth = &g_this->info.parameters[6];
    const Parameter* p_blend_mode = &g_this->info.parameters[7];
    const Parameter* p_show_light_pos = &g_this->info.parameters[8];
    const Parameter* p_invert_depth = &g_this->info.parameters[9];
    ...
    switch (uMsg) {
        case WM_INITDIALOG: {
            ...
            init_ranged_slider(
                p_depth, g_this->get_int(p_depth), hwndDlg, IDC_DEPTH, 10);
            init_ranged_slider(p_on_beat_depth,
                               g_this->get_int(p_on_beat_depth),
                               hwndDlg,
                               IDC_DEPTH2,
                               10);
            init_ranged_slider(p_on_beat_duration,
                               g_this->get_int(p_on_beat_duration),
                               hwndDlg,
                               IDC_BEATDUR,
                               10);
            ...
            SendDlgItemMessage(hwndDlg,
                               IDC_COMBO1,
                               CB_SETCURSEL,
                               (WPARAM)g_this->get_int(p_depth_buffer),
                               0);
            return 1;
        }
        case WM_NOTIFY: {
            if (element == IDC_DEPTH) {
                g_this->set_int(
                    p_depth, SendDlgItemMessage(hwndDlg, IDC_DEPTH, TBM_GETPOS, 0, 0));
            }
