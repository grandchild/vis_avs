    ...
    switch (uMsg) {
        case WM_INITDIALOG:
            ...
            SendDlgItemMessage(hwndDlg,
                               ...
                               TBM_SETPOS,
                               TRUE,
                               (int)(g_ConfigThis->WarpSpeed * 100));
            SendDlgItemMessage(hwndDlg, IDC_NUMSTARS, TBM_SETTICFREQ, 100, 0);
            SendDlgItemMessage(
                hwndDlg, IDC_NUMSTARS, TBM_SETRANGE, TRUE, MAKELONG(100, 4095));
            SendDlgItemMessage(
                hwndDlg, IDC_NUMSTARS, TBM_SETPOS, TRUE, g_ConfigThis->MaxStars_set);
            SendDlgItemMessage(hwndDlg, IDC_SPDCHG, TBM_SETTICFREQ, 10, 0);
            SendDlgItemMessage(
                hwndDlg, IDC_SPDCHG, TBM_SETRANGE, TRUE, MAKELONG(1, 5000));
            SendDlgItemMessage(hwndDlg,
                               IDC_SPDCHG,
                               TBM_SETPOS,
                               TRUE,
                               (int)(g_ConfigThis->spdBeat * 100));
            SendDlgItemMessage(hwndDlg, IDC_SPDDUR, TBM_SETTICFREQ, 10, 0);
            SendDlgItemMessage(
                hwndDlg, IDC_SPDDUR, TBM_SETRANGE, TRUE, MAKELONG(1, 100));
            SendDlgItemMessage(
                hwndDlg, IDC_SPDDUR, TBM_SETPOS, TRUE, (int)(g_ConfigThis->durFrames));
        ...
        case WM_NOTIFY: {
            if (LOWORD(wParam) == IDC_SPDCHG)
                g_ConfigThis->spdBeat =
                    (float)SendDlgItemMessage(hwndDlg, IDC_SPDCHG, TBM_GETPOS, 0, 0)
                    / 100;
            if (LOWORD(wParam) == IDC_SPDDUR)
                g_ConfigThis->durFrames =
                    SendDlgItemMessage(hwndDlg, IDC_SPDDUR, TBM_GETPOS, 0, 0);
            if (LOWORD(wParam) == IDC_SPEED) {
                g_ConfigThis->WarpSpeed =
                    (float)SendDlgItemMessage(hwndDlg, IDC_SPEED, TBM_GETPOS, 0, 0)
                    / 100;
                g_ConfigThis->CurrentSpeed =
                    (float)SendDlgItemMessage(hwndDlg, IDC_SPEED, TBM_GETPOS, 0, 0)
                    / 100;
            }
