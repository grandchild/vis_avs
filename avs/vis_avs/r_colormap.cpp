...
int C_ColorMap::get_new_id() { return this->next_id++; }
...
void C_ColorMap::bake_full_map(int map_index) {
    ...
    for (; color_index < this->maps[map_index].length - 1; color_index++) {
        ...
        for (; cache_index < to.position; cache_index++) {
            ...
            this->baked_maps[map_index].colors[cache_index] =
...
void C_ColorMap::sort_colors(int map_index) {
    qsort(this->maps[map_index].colors,
          this->maps[map_index].length,
          sizeof(map_color),
          compare_colors);
}
...
inline void C_ColorMap::animation_step() {
    for (unsigned int i = 0; i < NUM_COLOR_VALUES; i++) {
        this->tween_map.colors[i] =
            BLEND_ADJ_NOMMX(this->baked_maps[this->next_map].colors[i],
...
inline void C_ColorMap::animation_step_sse2() {
    for (unsigned int i = 0; i < NUM_COLOR_VALUES; i += 4) {
        ...
        // _mm_loadu_si128((__m128i*)&(this->baked_maps[this->current_map].colors[i]));
        // __m128i four_next_values =
        // _mm_loadu_si128((__m128i*)&(this->baked_maps[this->next_map].colors[i]));
        // __m128i blend_lerp = _mm_set1_epi8((unsigned
        // char)this->change_animation_step);
        /* TODO */
    }
    return;
}
...
int C_ColorMap::render(char visdata[2][2][576],
                       int is_beat,
                       int* framebuffer,
                       int* fbout,
                       int w,
...
char* C_ColorMap::get_desc(void) { return MOD_NAME; }
...
bool C_ColorMap::load_map_header(unsigned char* data, int len, int map_index, int pos) {
    ...
    pos += 4;
    strncpy(this->maps[map_index].filename,
            (char*)&data[pos],
            COLORMAP_MAP_FILENAME_MAXLEN - 1);
...
int C_ColorMap::save_config(unsigned char* data) {
    ...
    for (int map_index = 0; map_index < COLORMAP_NUM_MAPS; map_index++) {
        ...
        strncpy((char*)&data[pos],
                this->maps[map_index].filename,
                COLORMAP_MAP_FILENAME_MAXLEN - 1);
