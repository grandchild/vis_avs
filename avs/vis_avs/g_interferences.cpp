...
int win32_dlgproc_interferences(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM) {
    ...
    const Parameter* p_separate_rgb = &g_this->info.parameters[1];
    ...
    const Parameter* p_on_beat = &g_this->info.parameters[6];
    ...
    const Parameter* p_blend_mode = &g_this->info.parameters[11];

    auto element = LOWORD(wParam);
    switch (uMsg) {
        case WM_INITDIALOG: {
            auto num_layers = g_this->get_int(p_num_layers);
            init_ranged_slider(p_num_layers, num_layers, hwndDlg, IDC_NPOINTS);
            auto alpha = g_this->get_int(p_alpha);
            ...
            auto rotation = -g_this->get_int(p_rotation) - p_rotation->int_min;
            init_ranged_slider(p_rotation, rotation, hwndDlg, IDC_ROTATE);
            auto on_beat_alpha = g_this->get_int(p_on_beat_alpha);
            init_ranged_slider(p_on_beat_alpha, on_beat_alpha, hwndDlg, IDC_ALPHA2);
            auto on_beat_distance = g_this->get_int(p_on_beat_distance);
            init_ranged_slider(
                p_on_beat_distance, on_beat_distance, hwndDlg, IDC_DISTANCE2);
            auto on_beat_rotation =
                -g_this->get_int(p_on_beat_rotation) - p_on_beat_rotation->int_min;
            init_ranged_slider(
                p_on_beat_rotation, on_beat_rotation, hwndDlg, IDC_ROTATE2);
            auto init_rotation =
                p_init_rotation->int_max - g_this->get_int(p_init_rotation);
            init_ranged_slider(p_init_rotation, init_rotation, hwndDlg, IDC_INITROT);

            auto speed = g_this->get_float(p_on_beat_speed);
            ...
            EnableWindow(GetDlgItem(hwndDlg, IDC_RGB),
                         g_this->get_int(p_num_layers) % 3 == 0);
        ...
        case WM_NOTIFY:
            ...
            if (element == IDC_ALPHA) {
                g_this->set_int(
                    p_alpha, SendDlgItemMessage(hwndDlg, IDC_ALPHA, TBM_GETPOS, 0, 0));
            }
