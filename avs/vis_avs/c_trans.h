...
typedef struct {
    //    int index;       // Just here for descriptive purposes, makes it easy to match
    //    stuff up.
    char* list_desc;  // The string to show in the listbox.
    char* eval_desc;  // The optional string to display in the evaluation editor.
    char uses_eval;   // If this is true, the preset engages the eval library and there
                      // is NULL in the radial_effects array for its entry
...
class C_THISCLASS : public C_RBASE2 {
   ...
   public:
    ...
    char* help_text =
        ...
        "(0..2PI).\r\n"
        "You can also access 'sw' and 'sh' for screen dimensions in pixels (might be "
        "useful)\r\n";
    ...
    Description descriptions[24] = {
        ...
        {/*10,*/ "bubbling outward with swirl",
         "t = sin(d * $PI);\r\nd = d - "
         "(8*t*t*t*t*t)/sqrt((sw*sw+sh*sh)/4);\r\nt=cos(d*$PI/2.0);\r\nr= r + "
         "0.1*t*t*t;",
         0,
         0},
        {/*11,*/ "5 pointed distro",
        ...
        {/*14,*/ "shifted big swirl out",
         "// this is a very bad approximation in script. fixme.\r\nd=sqrt(x*x+y*y); "
