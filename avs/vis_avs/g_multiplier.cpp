    ...
    switch (uMsg) {
        case WM_COMMAND:
            if (HIWORD(wParam) == BN_CLICKED) {
                id = LOWORD(wParam);
                switch (id) {
                    case IDC_XI: g_this->config.multiply = MULTIPLY_XI; break;
                    ...
                    case IDC_XS: g_this->config.multiply = MULTIPLY_XS; break;
                }
            }
            return 0;
        ...
        case WM_INITDIALOG:
            switch (g_this->config.multiply) {
                case MULTIPLY_XI: id = IDC_XI; break;
                ...
                case MULTIPLY_XS: id = IDC_XS; break;
            }
            if (id) {
