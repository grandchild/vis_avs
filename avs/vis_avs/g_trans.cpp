...
int win32_dlgproc_movement(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM) {
    ...
    switch (uMsg) {
        case WM_INITDIALOG: {
            unsigned int x;
            for (x = 0;
                 x < sizeof(g_this->descriptions) / sizeof(g_this->descriptions[0]);
                 x++) {
                SendDlgItemMessage(hwndDlg,
                                   IDC_LIST1,
                                   LB_ADDSTRING,
                                   0,
                                   (long)(g_this->descriptions[x].list_desc));
            }

            isstart = 1;
            SetDlgItemText(hwndDlg, IDC_EDIT1, (char*)g_this->effect_exp.c_str());
            // After we set whatever value into the edit box, that's the new saved value
            // (ie: don't change the save format)
            isstart = 0;

            if (g_this->blend) CheckDlgButton(hwndDlg, IDC_CHECK1, BST_CHECKED);
            if (g_this->subpixel) CheckDlgButton(hwndDlg, IDC_CHECK4, BST_CHECKED);
            if (g_this->wrap) CheckDlgButton(hwndDlg, IDC_WRAP, BST_CHECKED);
            if (g_this->rectangular) CheckDlgButton(hwndDlg, IDC_CHECK3, BST_CHECKED);
            ...
            SendDlgItemMessage(
                hwndDlg, IDC_LIST1, LB_ADDSTRING, 0, (long)"(user defined)");
            SendDlgItemMessage(
                hwndDlg,
                IDC_LIST1,
                LB_SETCURSEL,
            ...
            if (g_this->effect == 32767) {
                ...
                if (strlen(g_this->descriptions[g_this->effect].eval_desc) > 0) {
                    SetDlgItemText(hwndDlg,
                                   IDC_EDIT1,
                                   g_this->descriptions[g_this->effect].eval_desc);
                    CheckDlgButton(hwndDlg,
                                   IDC_CHECK3,
                                   g_this->descriptions[g_this->effect].uses_rect
                                       ? BST_CHECKED
                                       : 0);
                    EnableWindow(GetDlgItem(hwndDlg, IDC_EDIT1), 1);
                    EnableWindow(GetDlgItem(hwndDlg, IDC_CHECK3), 1);
        ...
        case WM_COMMAND:
            if (LOWORD(wParam) == IDC_EDIT1 && HIWORD(wParam) == EN_CHANGE) {
                ...
                // If someone edits the editwnd, force the "(user defined)" to be the
                // new selection.
            ...
            if (LOWORD(wParam) == IDC_LIST1 && HIWORD(wParam) == LBN_SELCHANGE) {
                ...
                if (t
                    ...
                    SetDlgItemText(
                        hwndDlg, IDC_EDIT1, (char*)g_this->effect_exp.c_str());
                    ...
                    if (strlen(g_this->descriptions[t].eval_desc) > 0) {
                        ...
                        SetDlgItemText(
                            hwndDlg, IDC_EDIT1, g_this->descriptions[t].eval_desc);
                        EnableWindow(GetDlgItem(hwndDlg, IDC_EDIT1), 1);
                        EnableWindow(GetDlgItem(hwndDlg, IDC_CHECK3), 1);
                        EnableWindow(GetDlgItem(hwndDlg, IDC_LABEL1), 1);
                        CheckDlgButton(
                            hwndDlg,
                            IDC_CHECK3,
                            g_this->descriptions[t].uses_rect ? BST_CHECKED : 0);
                    } else {
                        // otherwise, they're not editable.
                        CheckDlgButton(
                            hwndDlg, IDC_CHECK3, g_this->rectangular ? BST_CHECKED : 0);
