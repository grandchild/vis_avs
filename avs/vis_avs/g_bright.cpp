...
int win32_dlgproc_brightness(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (uMsg) {
        case WM_INITDIALOG:
            ...
            SendDlgItemMessage(
                hwndDlg, IDC_RED, TBM_SETPOS, TRUE, g_ConfigThis->redp + 4096);
            SendDlgItemMessage(
                hwndDlg, IDC_GREEN, TBM_SETPOS, TRUE, g_ConfigThis->greenp + 4096);
            SendDlgItemMessage(
                hwndDlg, IDC_BLUE, TBM_SETPOS, TRUE, g_ConfigThis->bluep + 4096);
            SendDlgItemMessage(
                hwndDlg, IDC_DISTANCE, TBM_SETRANGE, TRUE, MAKELONG(0, 255));
            SendDlgItemMessage(
        ...
        case WM_NOTIFY: {
            ...
            if (LOWORD(wParam) == IDC_RED) {
                g_ConfigThis->redp =
                    SendDlgItemMessage(hwndDlg, IDC_RED, TBM_GETPOS, 0, 0) - 4096;
            rred:
                ...
                if (!g_ConfigThis->dissoc) {
                    ...
                    SendDlgItemMessage(hwndDlg,
                                       ...
                                       TBM_SETPOS,
                                       TRUE,
                                       g_ConfigThis->greenp + 4096);
                    g_ConfigThis->bluep = g_ConfigThis->redp;
                    SendDlgItemMessage(hwndDlg,
                                       IDC_BLUE,
                                       TBM_SETPOS,
                                       TRUE,
                                       g_ConfigThis->bluep + 4096);
                }
            }
            if (LOWORD(wParam) == IDC_GREEN) {
                g_ConfigThis->greenp =
                    SendDlgItemMessage(hwndDlg, IDC_GREEN, TBM_GETPOS, 0, 0) - 4096;
            rgreen:
                g_ConfigThis->tabs_needinit = 1;
                if (!g_ConfigThis->dissoc) {
            ...
            if (LOWORD(wParam) == IDC_BLUE) {
                g_ConfigThis->bluep =
                    SendDlgItemMessage(hwndDlg, IDC_BLUE, TBM_GETPOS, 0, 0) - 4096;
            rblue:
                g_ConfigThis->tabs_needinit = 1;
                if (!g_ConfigThis->dissoc) {
        ...
        case WM_COMMAND:
            ...
            if ((LOWORD(wParam) == IDC_CHECK1) || (LOWORD(wParam) == IDC_ADDITIVE)
                || (LOWORD(wParam) == IDC_REPLACE) || (LOWORD(wParam) == IDC_EXCLUDE)
                || (LOWORD(wParam) == IDC_DISSOC) || (LOWORD(wParam) == IDC_5050)) {
                g_ConfigThis->enabled = IsDlgButtonChecked(hwndDlg, IDC_CHECK1) ? 1 : 0;
                g_ConfigThis->exclude =
                    IsDlgButtonChecked(hwndDlg, IDC_EXCLUDE) ? 1 : 0;
