...
int win32_dlgproc_avi(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM) {
    ...
    switch (uMsg) {
        case WM_INITDIALOG:
            ...
            SendDlgItemMessage(
                hwndDlg, IDC_SPEED, TBM_SETRANGE, TRUE, MAKELONG(0, 1000));
            SendDlgItemMessage(
                hwndDlg, IDC_SPEED, TBM_SETPOS, TRUE, g_ConfigThis->speed);
            ...
            if (!g_ConfigThis->adapt && !g_ConfigThis->blend
                && !g_ConfigThis->blendavg) {
                CheckDlgButton(hwndDlg, IDC_REPLACE, BST_CHECKED);
            }
            EnableWindows(hwndDlg, g_ConfigThis);
            loadComboBox(
                GetDlgItem(hwndDlg, OBJ_COMBO), "*.AVI", g_ConfigThis->ascName);
            return 1;
        case WM_NOTIFY:
