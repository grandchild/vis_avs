...
int win32_dlgproc_custombpm(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM) {
    ...
    switch (uMsg) {
        case WM_INITDIALOG: {
            ...
            SendDlgItemMessage(
                hwndDlg, IDC_ARBVAL, TBM_SETRANGE, TRUE, MAKELONG(200, 10000));
            SendDlgItemMessage(
                hwndDlg, IDC_SKIPVAL, TBM_SETRANGE, TRUE, MAKELONG(1, 16));
            SendDlgItemMessage(
                hwndDlg, IDC_ARBVAL, TBM_SETPOS, TRUE, g_ConfigThis->arbVal);
            SendDlgItemMessage(
                hwndDlg, IDC_SKIPVAL, TBM_SETPOS, TRUE, g_ConfigThis->skipVal);
            ...
            SendDlgItemMessage(
                hwndDlg, IDC_SKIPFIRST, TBM_SETRANGE, TRUE, MAKELONG(0, 64));
            SendDlgItemMessage(
                hwndDlg, IDC_SKIPFIRST, TBM_SETPOS, TRUE, g_ConfigThis->skipfirst);
            wsprintf(txt, "%d bpm", 60000 / g_ConfigThis->arbVal);
            SetDlgItemText(hwndDlg, IDC_ARBTXT, txt);
            wsprintf(txt,
                     "%d beat%s",
                     g_ConfigThis->skipVal,
                     g_ConfigThis->skipVal > 1 ? "s" : "");
            SetDlgItemText(hwndDlg, IDC_SKIPTXT, txt);
            wsprintf(txt,
                     "%d beat%s",
                     g_ConfigThis->skipfirst,
                     g_ConfigThis->skipfirst > 1 ? "s" : "");
        ...
        case WM_TIMER: {
            if (g_ConfigThis->oldInSlide != g_ConfigThis->inSlide) {
                SendDlgItemMessage(
                    hwndDlg, IDC_IN, TBM_SETPOS, TRUE, g_ConfigThis->inSlide);
                g_ConfigThis->oldInSlide = g_ConfigThis->inSlide;
            }
            if (g_ConfigThis->oldOutSlide != g_ConfigThis->outSlide) {
                SendDlgItemMessage(
                    hwndDlg, IDC_OUT, TBM_SETPOS, TRUE, g_ConfigThis->outSlide);
        ...
        case WM_COMMAND:
            if ((LOWORD(wParam) == IDC_CHECK1) || (LOWORD(wParam) == IDC_ARBITRARY)
                ...
                g_ConfigThis->arbitrary =
                    IsDlgButtonChecked(hwndDlg, IDC_ARBITRARY) ? 1 : 0;
