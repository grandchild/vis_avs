...
int win32_dlgproc_dynamicdistancemodifier(HWND hwndDlg,
                                          ...
                                          WPARAM wParam,
                                          LPARAM lParam) {
    E_DynamicDistanceModifier* g_this = (E_DynamicDistanceModifier*)g_current_render;
    ...
    const AVS_Parameter_Handle p_blend_mode = g_this->info.parameters[4].handle;
    const AVS_Parameter_Handle p_bilinear = g_this->info.parameters[5].handle;
    ...
    switch (uMsg) {
        case WM_INITDIALOG:
            ...
            SetDlgItemText(hwndDlg, IDC_EDIT4, (char*)g_this->get_string(p_point));
            isstart = 0;
