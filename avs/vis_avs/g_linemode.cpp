...
int win32_dlgproc_setrendermode(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM) {
    ...
    switch (uMsg) {
        case WM_INITDIALOG: {
            unsigned int x;
            for (x = 0; x < sizeof(g_this->line_blendmodes)
                                / sizeof(g_this->line_blendmodes[0]);
                 x++) {
            ...
            SendDlgItemMessage(hwndDlg,
                               ...
                               TBM_SETPOS,
                               TRUE,
                               (int)(g_this->newmode >> 8) & 0xff);
            SendDlgItemMessage(
                hwndDlg, IDC_COMBO1, CB_SETCURSEL, (WPARAM)g_this->newmode & 0xff, 0);
            EnableWindow(GetDlgItem(hwndDlg, IDC_OUTSLIDE),
                         (g_this->newmode & 0xff) == 7);
        ...
        case WM_COMMAND:
            switch (LOWORD(wParam)) {
                ...
                case IDC_CHECK1:
                    ...
                    g_this->newmode |=
                        IsDlgButtonChecked(hwndDlg, IDC_CHECK1) ? 0x80000000 : 0;
                    break;
                case IDC_COMBO1:
